...
CSDR порядок (**Conditions, Space, Distribution и Realization**) применим в любом языке программирования и позволяет создавать модульный код.
...
1. <a href="#Модули">Модули</a>
    ...
    1.7. <a href="#Роли">Роли</a>
    
    1.8. <a href="#Выполнение">Выполнение</a>
...
2. <a href="#Процессор">Процессор</a>
    ...
    2.3. <a href="#Алгоритм-регуляции">Алгоритм регуляции</a>
...
Замыслом будет модуль и всё что требуется от заказчика, так это название модуля и перечисление необходимых конструкций в нём.
...
4. Автозапуск, где отмечается первоочередный запуск при запуске других конструкций
...
В местах кода можно вызвать для выполнения определенный модуль, обозначив: {название модуля}, {назначение конструкции}. При этом, в местах можно вызвать выполнение конструкцию этого же модуля, обозначив {название модуля = @this}.

Пример модуля (<a target="_blank" href="https://github.com/it-architector/code.csdr/blob/master/%D0%9A%D0%B0%D1%80%D1%82%D0%B8%D0%BD%D0%BA%D0%B8/%D0%92%D0%B7%D0%B0%D0%B8%D0%BC%D0%BE%D0%B4%D0%B5%D0%B9%D1%81%D1%82%D0%B2%D0%B8%D0%B5%20%D1%8F%D0%B4%D1%80%D0%B0.png?raw=true">открыть в отдельном окне</a>):

![---------------------](./Картинки/Взаимодействие%20ядра.png)

Для начального кодирования введём модули **процессор** с такими конструкциями:
