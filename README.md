...
Выполнять вышеописанный цикл будет функция процессора, в которой будет такой функционал:

1. **[по запросу: сообщение о фатальной ошибке]** die с текстом
2. **[по запросу: проверка прав]** проверка успешного выполнения алгоритма права
3. **[по запросу: рефлекс]** берём тип, алгоритм права, сценарий, вложения у рефлекса по id
   1. запрашиваем проверку прав
   2. **[есть тип ожидание прав единожды / множественно]** проверка наличия в массиве ожидания прав
      1. **[нет наличия в массиве ожидания прав]** добавляем id, алгоритм права рефлекса в массив ожидания прав
      2. **[есть наличие в массиве ожидания прав] [есть права] [есть тип ожидание прав единожды]** удаляем рефлекс из массива ожидания прав
   3. **[есть права] [цикл]** последовательно берём тип, команду, id реакции, id связи у мест из сценария
      1. **[есть реакция]** берём тип у реакции
         1. **[есть тип в роли]** выполнение функции из роли
         2. **[есть тип вручную]** выполнение функции реакции
      2. **[есть связь]** берём тип, массив возможностей у связи
         1. **[есть тип внешнее в место]** выполнение функции передачи информации с внешнего источника в места
         2. **[есть тип место в реакцию]** выполнение функции передачи информации с мест в реакцию
         3. **[есть тип реакция в место]** выполнение функции передачи информации с реакции в места
         4. **[есть тип место во внешнее]** выполнение функции передачи информации с мест во внешний источник
   3. **[цикл]** последовательно запрашиваем рефлексы во вложении
4. **[по запросу: ожидание прав]** проверка наличия массива ожидания прав
   1. **[есть массив ожидания прав] [цикл]** последовательно берём id, алгоритмы прав из массива ожидания прав
      1. запрашиваем проверку прав
          1. **[есть права]** запрашиваем рефлекс
   2. запрос ожидания прав
5. **[цикл]** последовательно берём обозначение технологий проекта
   1. проверяем наличие технологии
      1. **[нет наличия технологии]** добавить технологию в массив где нет технологий
6. проверяем наличие массива где нет технологий
7. **[есть наличие массива где нет технологий]** запросить сообщение о фатальной ошибке с текстом о необходимых технологиях
8. **[нет наличия массива где нет технологий]** 
   1. запрашиваем рефлекс с id = 1
   2. запрашиваем ожидание прав
