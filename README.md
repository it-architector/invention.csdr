...
Применяя CSDR порядок (**Conditions, Space, Distribution и Realization**) вы будете уверены в том, что код будет модульным. При этом код составляются последовательными типизированными шагами, что позволяет работать над ним нескольких программистам, одновременно, исключая возникновения конфликтов.
