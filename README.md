...
5. <a href="#Прототип-проектов-на-PHP">Прототип проектов на PHP</a>
...
6. <a href="#Прототип-проектов-на-JS">Прототип проектов на JS</a>
...
1. **[по запросу: память ожидания прав] [входящее: {запрос = добавить, проверить, удалить, получить всё}, {id ожидающего рефлекса}]** добавление {id рефлекса}, {алгоритм права} / получить всё {массива ожидания прав} / проверить / удаление в массиве "ожидания прав" у процессора
2. **[по запросу: информация о рефлексе] [входящее: {id рефлекса}]** получение {тип рефлекса}, {алгоритм права}, {сценарий ролей}, {id вложенных рефлексов} в рефлексах проекта
3. **[по запросу: информация о месте] [входящее: {id места}]** получение {тип места}, {команда}, {id реакции}, {id связи} в местах проекта
4. **[по запросу: информация о реакции] [входящее: {id реакции}]** получение {тип реакции}, {функция в роли}, {функция реакции} в проекте
5. **[по запросу: информация о связи] [входящее: {id связи}]** получение {тип связи}, {массив возможностей} в связях проекта
6. **[по запросу: передача информации с внешнего источника в места] [входящее: {массив возможностей}]** изменение значений в местах проекта
7. **[по запросу: передача информации с мест в реакцию] [входящее: {массив возможностей}]** изменение значений в реакциях проекта
8. **[по запросу: передача информации с реакции в места] [входящее: {массив возможностей}]** изменение значений в местах проекта
9. **[по запросу: передача информации с мест во внешний источник] [входящее: {массив возможностей}]** передача значений в местах проекта
10. **[по запросу: проверка прав] [входящее: {алгоритм права}]** проверка успешного выполнения алгоритма права для мест проекта
11. **[по запросу: активация рефлекса] [входящее: {id рефлекса}]** запрос "*информация о рефлексе*" по {id рефлекса}
...
12. **[по запросу: проверка прав у ожидающих рефлексов]** запрос "*память ожидания прав: получить всё*"
...
13. запрос "*активация рефлекса*" с {id рефлекса = 1}
...
$project['Места']    = [
    '1' => [
        'Смысл'    => 'Описание 1',
        'Роль'     => ['Категория роли 1'=>['Название роли 1'=>'Команда роли 1']],
        'Связь'    => 'Наличие связи 1',
        'Реакция'  => 'Наличие реакции 1',
        'Вложение' => [
            '1.1' => [
                'Смысл'    => 'Описание 2',
                'Роль'     => ['Категория роли 2'=>['Название роли 2'=>'Команда роли 2']],
                'Связь'    => 'Наличие связи 2',
                'Реакция'  => 'Наличие реакции 2',
                'Вложение' => [
                    '1.1.1' => [
                        'Смысл'    => 'Описание 3',
                        'Роль'     => ['Категория роли 3'=>['Название роли 3'=>'Команда роли 3']],
                    ...
                    '1.1.2' => [
                        'Смысл'    => 'Описание 4',
                        'Роль'     => ['Категория роли 4'=>['Название роли 4'=>'Команда роли 4']],
...
Код всех ролей, которые применяются в проекте.
...
Вам это знакомо как модули с проработанным api, что поможет собирать проект как конструктор. Роль состоит из готового исходника кода проекта CSDR с возможностью запуска в процессоре. 

Обозначаются роли: {категорией}, {названием} и {командой}.

**Категория роли** нужна для концентрации проектирования вовне роли:
1. Хранение
> форма вмещения
> 
...
**Название роли** это название данного проекта.

**Команды роли** (может и не быть) нужна для концентрации проектирования внутри роли. 
...
5. Контроль: реакция: получение, передача
> благодаря которой проект сможет взаимодействовать с реакциями
6. Контроль: запрос: команда, получение, передача
> благодаря которой проект сможет взаимодействовать с внешними проектами


![---------------------](./Картинки/hr.png)

<h2>Прототип проектов на PHP</h2>
