...
8. **[по запросу: активация рефлекса] [входящее: {id рефлекса}]** запрос "*информация о рефлексе*" по {id рефлекса}
    ...
    3. **[есть права] [цикл]** последовательный запрос "*информация о месте*" у мест из {сценарий ролей}
       ...
       3. запрос "*процессор*" с {проектом роли}
       4. запрос "*проверка прав у ожидающих рефлексов*"
    3. **[цикл]** последовательный запрос "*активация рефлекса*" с {id рефлекса} во вложении
9. **[по запросу: процессор] [входящее: {проект}, {запрос}, {параметры запроса}]** запрос "*активация рефлекса*" с {id рефлекса = 1}
10. запрос "*процессор*" с {проектом}
...
Код принято делать в виде 6-и массивов:
...
**Параметры** принято делать фиксировано:
...
$project['Параметры'] = [
    ...
    'Версия' => '{correct fix}',
    'Запросы' => [
               '{Запрос 1}' => '{Параметры запроса 1}', 
               '{Запрос 2}' => '{Параметры запроса 2}',
    ],
];
```

> Где, название это название самого проекта.
> 
> Где, версия это номер опубликованной корректировки проекта.
> 
> Где, в запросах перечислен api к проекту.
...
$cpu = [
    'Версии' => [
       '{Версия процессора 1}' => function('{проект}','{запрос}','{параметры запроса}'){...},
       '{Версия процессора 2}' => function('{проект}','{запрос}','{параметры запроса}'){...},
     ],
    'Роли' => [
         'Хранение'   => [...],
         'Содержание' => [...],
...
> Где, в версиях все процессоры, под которые были написаны проекты. 
> 
> Где, все применённые в проекте роли.
...
```php
$cpu['Версии']['{Версия процессора}']('{проект}','{запрос}','{параметры запроса}');
```
...
Обозначаются роли: {категорией}, {названием} и {запросами}.
...
**Запрос роли** (может и не быть) нужна для концентрации проектирования внутри роли. 
