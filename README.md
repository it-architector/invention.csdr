...
2. Процессор, где функционал который выполнит модули (вам это знакомо как микросервисы)
...
В каждом месте можно вызвать для выполнения определенный модуль, обозначив: {название модуля}, {версия модуля}, {назначение конструкции}. 

При этом, в местах можно вызвать выполнение конструкции этого же модуля, обозначив {название модуля = @this}. Это даст возможность создать рекрусию (аналог while).
...
10. **[по запросу: активация процессора] [входящее: {название модуля}, {версия модуля}, {название конструкции}, {параметры конструкции}]** запрос "*активация конструкции*"
