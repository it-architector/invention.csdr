...
5. Api, где точки соприкосновения с конструкцией
...
5. Ожидание, где варианты ожидания получения прав со временем: нет, да 
...
- api ответа: {допустимые модели}, {допустимые роли}, {допустимые конструкции}
2. Содержание
- позволяет для мест: ресурсы
- api запроса: {действие = активировать, получить, отдать}, {ресурс}
- api ответа: {ресурс}
3. Создание
- позволяет для мест: реакции
- api запроса: {переменные с ресурсами}
- api ответа: {переменные с ресурсами}
4. Контакт
- позволяет для мест: связи
- api запроса: {переменные откуда в куда}
5. Автозапуск
- запускается вначале при каждом запуске других конструкций
- позволяет для мест: не применим
