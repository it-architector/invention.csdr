...
Применяя проект CSDR (**Conditions, Space, Distribution и Realization**) в любом языке программирования, можно быть уверенным, что код проекта будет конструктивным.
...
1. **[по запросу]** проверка технологий
2. **[цикл]** последовательно берём id рефлексов проекта, где права верны
   1. **[по событию] [цикл]** последовательно берём id мест из сценария рефлекса по id
      1. **[по запросу]** берём информацию места по id
            1. **[по запросу]** ставим рефлекс на событие
            2. **[по запросу]** выполнение функции из роли
            3. **[по запросу]** выполнение функции из реакции
            4. **[по запросу]** выполнение функции связи
...
```php
/*функция процессора*/
$project['Процессор'] = function($project){
};
```
