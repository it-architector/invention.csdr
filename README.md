...
Применяя CSDR порядок (**Conditions, Space, Distribution и Realization**) вы будете уверены в том, что код будет модульным. При этом код программируется последовательными типизированными шагами, что позволяет одновременно работать над ним нескольким программистам по разным задачам, исключая возникновения конфликтов в виде "стиля программирования" на 100%.
